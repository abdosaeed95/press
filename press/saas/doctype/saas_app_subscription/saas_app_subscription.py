...
class SaasAppSubscription(Document):
        ...
        if "email_delivery_service" in [
            ...
            new_config.extend(
                [
                    ...
                    {"key": "mail_login",
                     "value": "example@gmail.com", "type": "String"},
                    {"key": "mail_password",
                     "value": "password", "type": "String"},
                    {"key": "mail_port", "value": 587, "type": "Number"},
                    {"key": "mail_server",
                     "value": "smtp.gmail.com", "type": "String"},
                ]
            )
    ...
    def validate_saas_app_plan(self):
        ...
        if app != self.app:
            frappe.throw(
                f"Plan {self.saas_app_plan} is not for app {frappe.bold(self.app)}!")
    ...
    def change_plan(self, new_plan, ignore_card_setup=False):
        ...
        if self.app == "erpnext_smb":
            ...
            config = json.loads(
                frappe.db.get_value(
                    "Saas App Plan", self.saas_app_plan, "config")
            )
    ...
    def update_end_date(self, payment_option):
        ...
        self.end_date = add_to_date(
            self.end_date or datetime.today().date(), days=days)
...
def should_create_usage_record(subscription):
    ...
    is_free = frappe.db.get_value(
        "Saas App Plan", subscription.saas_app_plan, "is_free")
