...
def setup_root_domain(settings):
    ...
    while not frappe.db.exists(
            "TLS Certificate", {"wildcard": True,
                                "domain": ROOT_DOMAIN, "status": "Active"}
    ):
...
def setup_teams():
    ...
    cloud = Team.create_new(request, "Frappe", "Cloud",
                            "FrappeCloud@1", "India", False)

    signup("aditya@erpnext.com")
    request = frappe.get_all(
        "Account Request", ["*"], {"email": "aditya@erpnext.com"}, limit=1
    )[0]
    aditya = Team.create_new(request, "Aditya", "Hase",
...
def setup_apps():
    app = frappe.get_doc(
        {"doctype": "App", "name": "frappe",
            "title": "Frappe Framework", "frappe": True}
    ).insert(ignore_permissions=True)
