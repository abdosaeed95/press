...
class BenchFieldCheck(Audit):
    ...
    def generate_press_map(self):
        frappe.db.commit()
        sites = frappe.get_all("Site", ["name", "bench"], {
                               "status": ("!=", "Archived")})
        ...
        for replica, primary in replicas_and_primary:
            ...
            for bench, bench_desc in primary_benches.items():
                ...
                if sites_on_primary_only:
                    status = "Failure"
                    log[bench] = {
                        "Sites on primary only": sites_on_primary_only}
        self.log(log, status)
...
class BackupRecordCheck(Audit):
    ...
    def __init__(self):
        ...
        trial_plans = tuple(frappe.get_all(
            "Plan", dict(is_trial_plan=1), pluck="name"))
        ...
        sites_without_backups = all_sites.difference(
            sites_with_backup_in_interval)
