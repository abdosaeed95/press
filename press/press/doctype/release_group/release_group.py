...
class ReleaseGroup(Document):
    ...
    def create_deploy_candidate(self, apps_to_ignore=None):
        ...
        last_deployed_bench = get_last_doc(
            "Bench", {"group": self.name, "status": "Active"})

        if last_deployed_bench:
            for app in untouched_apps:
                update = find(last_deployed_bench.apps,
            ...
            for bench_app in bench_apps:
                if not find(self.apps, lambda rg_app: rg_app.app == bench_app.app):
                    app_title = frappe.db.get_value(
                        "App", bench_app.app, "title")
                    removed_apps.append(
                        {"name": bench_app.app, "title": app_title})
    ...
    def change_app_branch(self, app: str, to_branch: str) -> None:
        ...
        required_app_source = frappe.get_all(
            "App Source",
            filters={
                "repository_url": current_app_source.repository_url, "branch": to_branch},
            limit=1,
        )
    ...
    def get_marketplace_app_sources(self) -> List[str]:
        all_marketplace_sources = frappe.get_all(
            "Marketplace App Version", pluck="source")
