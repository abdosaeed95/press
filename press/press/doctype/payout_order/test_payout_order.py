...
class TestPayoutOrder(FrappeTestCase):
    ...
    def create_test_inr_invoice(self):
        ...
        test_mp_app = create_test_marketplace_app(
            test_app.name, self.test_team.name)
    ...
    def test_create_marketplace_monthly_payout_order(self):
        ...
        self.assertFalse(frappe.db.exists("Payout Order", {
                         "recipient": self.test_team.name}))

        # Run the monthly job
        create_marketplace_payout_orders_monthly()

        # The Payout Order should have been created
        self.assertTrue(frappe.db.exists("Payout Order", {
                        "recipient": self.test_team.name}))
