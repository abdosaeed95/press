...
class DatabaseServer(BaseServer):
    ...
    def _trigger_failover(self):
        try:
            ansible = Ansible(
                ...
                server=self,
                variables={"mariadb_root_password": self.get_password(
            ...
            if play.status == "Success":
                ...
                servers = frappe.get_all(
                    "Server", {"database_server": old_primary})
            ...
            self.status = "Broken"
            log_error("Database Server Failover Exception",
                      server=self.as_dict())
        self.save(ignore_permissions=True)
    ...
    def _convert_from_frappe_server(self):
        ...
        try:
            ...
            self.status = "Broken"
            log_error("Database Server Conversion Exception",
                      server=self.as_dict())
        self.save(ignore_permissions=True)
    ...
    def reset_root_password_primary(self):
        ...
        try:
            ...
            log_error("Database Server Password Reset Exception",
                      server=self.as_dict())
            raise

    def reset_root_password_secondary(self):
        primary = frappe.get_doc("Database Server", self.primary)
        self.mariadb_root_password = primary.get_password(
            "mariadb_root_password")
        try:
            ansible = Ansible(
    ...
    def _rename_server(self):
        ...
        try:
            ...
            self.status = "Broken"
            log_error("Database Server Rename Exception",
                      server=self.as_dict())
        self.save(ignore_permissions=True)
