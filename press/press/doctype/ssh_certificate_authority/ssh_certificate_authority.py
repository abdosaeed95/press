...
class SSHCertificateAuthority(Document):
    ...
    def extract_key_fingerprint(self):
        self.key_fingerprint = self.run(
            "ssh-keygen -l -f ca.pub", directory=self.directory)
    ...
    def _prepare_build_context(self):
        for target in ["sshd_config", "Dockerfile"]:
            shutil.copy(
                os.path.join(frappe.get_app_path(
                    "press", "docker", "ssh_proxy"), target),
        ...
        host_key_path = os.path.join(
            self.build_directory, "ssh_host_rsa_key.pub")
    ...
    def _run_docker_build(self):
        ...
        self.run(f"docker build -t {self.docker_image} .",
                 self.build_directory, environment)
