...
class BaseServer(Document):
    ...
    def create_dns_record(self):
        try:
            ...
            client = boto3.client(
                ...
                aws_secret_access_key=domain.get_password(
            ...
            log_error("Route 53 Record Creation Error",
                      domain=domain.name, server=self.name)

    def validate_cluster(self):
        if not self.cluster:
            self.cluster = frappe.db.get_value(
                "Root Domain", self.domain, "default_cluster")
        if not self.cluster:
    ...
    def _prepare_server(self):
        try:
            if self.provider == "Scaleway":
                ...
                ansible = Ansible(playbook="aws.yml",
                                  server=self, user="ubuntu")
    ...
    def ping_ansible_unprepared(self):
        try:
            ...
            log_error("Unprepared Server Ping Exception",
                      server=self.as_dict())
    ...
    def subscription(self):
        name = frappe.db.get_value(
            "Subscription", {"document_type": self.doctype,
                             "document_name": self.name}
        )
    ...
    def change_plan(self, plan, ignore_card_setup=False):
        ...
        self.run_press_job("Resize Server", {
...
class Server(BaseServer):
    ...
    def _setup_secondary(self):
        primary_public_key = frappe.db.get_value(
            "Server", self.primary, "frappe_public_key")
        try:
            ansible = Ansible(
    ...
    def _auto_scale_workers_new(self):
        ...
        # avg ram usage of 3 sets of bg workers
        max_bg_workers = usable_ram_for_bg / (3 * 80)
        ...
        for bench_name, workload in bench_workloads.items():
            ...
            try:
                gunicorn_workers = min(
                    24, max(2, round(workload / total_workload *
                            max_gunicorn_workers))  # min 2 max 24
                )
                background_workers = min(
                    # min 1 max 8
                    8, max(1, round(workload / total_workload * max_bg_workers))
                )
...
def scale_workers():
    servers = frappe.get_all(
        "Server", {"status": "Active", "is_primary": True})
...
def cleanup_unused_files():
    servers = frappe.get_all(
        "Server", fields=["name"], filters={"status": "Active"})
    for server in servers:
        try:
