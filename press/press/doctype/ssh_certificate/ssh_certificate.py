...
class SSHCertificate(Document):
    ...
    def validate_public_key(self):
        try:
            ssh_key_b64 = base64.b64decode(
                self.ssh_public_key.strip().split()[1])
            sha256_sum = hashlib.sha256()
            sha256_sum.update(ssh_key_b64)
            self.ssh_fingerprint = base64.b64encode(
                sha256_sum.digest()).decode()
            self.key_type = self.ssh_public_key.strip().split()[
                0].split("-")[1]
        ...
        if not self.key_type:
            frappe.throw(
                "Could not guess the key type. Please check your public key.")

    def validate_validity(self):
        if self.certificate_type == "User" and self.validity not in ("1h", "3h", "6h"):
            frappe.throw(
                "User certificates can only be valid for a short duration")
    ...
    def validate_existing_certificates(self):
        if frappe.get_all(
                "SSH Certificate",
                {"user": self.user, "valid_until": [
                    ">", frappe.utils.now()], "group": self.group},
        ):
    ...
    def generate_certificate(self):
        ca = frappe.get_doc("SSH Certificate Authority",
    ...
    def extract_certificate_details(self):
        self.certificate_details = self.run(
            f"ssh-keygen -Lf {self.certificate_file}")
        regex = re.compile("Valid:.*\n")
        self.valid_until = regex.findall(self.certificate_details)[
            0].strip().split()[-1]
