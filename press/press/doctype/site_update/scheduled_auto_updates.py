...
def trigger():
    ...
    trigger_for_sites = list(
        filter(should_update_trigger, sites_with_scheduled_updates))
    ...
    for site in trigger_for_sites:
        ...
        try:
            ...
            log_error("Scheduled Auto Update Failed",
                      site=site, traceback=traceback)
        finally:
...
def should_update_trigger_for_monthly(doc, current_datetime=get_datetime()):
    ...
    if doc.update_end_of_month:
        on_day_of_month = get_last_day_of_month(
...
def set_schedule_details(update_log_doc, doc):
    ...
    if doc.update_trigger_frequency == "Weekly":
        ...
        update_log_doc.was_scheduled_for_month_day = str(
            doc.update_on_day_of_month)
