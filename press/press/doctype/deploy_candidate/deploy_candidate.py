...
class DeployCandidate(Document):
    ...
    def get_unpublished_marketplace_releases(self) -> List[str]:
        ...
        dc_app_releases = frappe.get_all(
            ...
            filters={"parent": self.name, "source": (
                "in", marketplace_app_sources)},
            pluck="release",
        )
        ...
        unpublished_releases = frappe.get_all(
            "App Release",
            filters={"name": ("in", dc_app_releases),
                     "status": ("!=", "Approved")},
            pluck="name",
        )
    ...
    def add_build_steps(self):
        ...
        preparation_steps = [
            ("pre", "essentials", "Setup Prerequisites",
        ...
        for app in self.apps:
            clone_steps.append(
                ("clone", app.app, "Clone Repositories", app.title))
            app_install_steps.append(
                ("apps", app.app, "Install Apps", app.title))
    ...
    def _prepare_build_directory(self):
        build_directory = frappe.get_value(
            "Press Settings", None, "build_directory")
        ...
        self.build_directory = os.path.join(
            build_directory, self.group, self.name)
    ...
    def _prepare_build_context(self):
        ...
        for app in self.apps:
            ...
            if cloned:
                ...
                end_time = now()
                step.duration = frappe.utils.rounded(
    ...
    def _generate_dockerfile(self):
        ...
        with open(dockerfile, "w") as f:
            ...
            content = frappe.render_template(
                dockerfile_template, {"doc": self}, is_path=True)
            f.write(content)

    def _copy_config_files(self):
        for target in ["common_site_config.json", "supervisord.conf"]:
            shutil.copy(
                os.path.join(frappe.get_app_path("press", "docker"),
                             target), self.build_directory
            )
    ...
    def _generate_redis_cache_config(self):
        redis_cache_conf = os.path.join(
            self.build_directory, "config", "redis-cache.conf")
    ...
    def _get_app_name(self, app):
        ...
        if not app_name:
            ...
            with open(setup_py_path, "rb") as f:
                app_name = re.search(
                    r'name\s*=\s*[\'"](.*)[\'"]', f.read().decode("utf-8"))[1]
    ...
    def _run_docker_build(self, no_cache=False):
        ...
        result = self.run(
            f"docker buildx build --platform linux/amd64 {'--no-cache' if no_cache else ''} -t {self.docker_image} .",
            environment,
        )
                ...
                if step_index not in steps and line.startswith("[stage-"):
                    ...
                    if name.startswith("RUN") and match:
                        ...
                        name = name.replace(match.group(0), "").strip().replace(
                            "   ", " \\\n  ")[4:]
                        stage_slug, step_slug = match.group(
                            1).split("-", maxsplit=1)
                        step = find(
                            self.build_steps,
    ...
    def _push_docker_image(self):
        ...
        try:
            ...
            settings = frappe.db.get_value(
                "Press Settings",
                None,
                ["docker_registry_url", "docker_registry_username",
    ...
    def generate_ssh_keys(self):
        ca = frappe.get_value("Press Settings", None,
    ...
    def generate_host_keys(self, ca, ssh_directory):
        ...
        host_public_key_path = os.path.join(
            ssh_directory, "ssh_host_rsa_key.pub")
        ca.sign(self.name, None, "+52w",
                host_public_key_path, 0, host_key=True)
    ...
    def create_deploy(self, staging: bool = False):
        ...
        if staging:
            ...
            if not servers:
                frappe.log_error(
                    title="Staging Server for new benches not found")
        else:
            ...
            deploy_doc = frappe.db.exists(
                "Deploy", {"group": self.group,
                           "candidate": self.name, "staging": False}
            )
