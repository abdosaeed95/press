...
def poll_file_statuses():
    ...
    default_region = frappe.db.get_single_value(
...
def delete_remote_backup_objects(remote_files):
    ...
    buckets = {bucket: []
               for bucket in frappe.get_all("Backup Bucket", pluck="name")}
    buckets.update({frappe.db.get_single_value(
        "Press Settings", "aws_s3_bucket"): []})
