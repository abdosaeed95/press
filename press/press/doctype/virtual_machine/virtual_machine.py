...
class VirtualMachine(Document):
    ...
    def validate(self):
        ...
        if self.virtual_machine_image:
            self.disk_size = max(
                self.disk_size,
                frappe.db.get_value("Virtual Machine Image",
                                    self.virtual_machine_image, "size"),
            )
    ...
    def get_cloud_init(self):
        ...
        init = frappe.render_template(
            cloud_init_template, context, is_path=True)
        return init

    def get_server(self):
        for doctype in ["Server", "Database Server"]:
            server = frappe.db.get_value(
                doctype, {"virtual_machine": self.name}, "name")
            if server:
    ...
    def update_servers(self):
        ...
        for doctype in ["Server", "Database Server", "Proxy Server"]:
            server = frappe.get_all(
                doctype, {"virtual_machine": self.name}, pluck="name")
            if server:
                server = server[0]
                frappe.db.set_value(doctype, server, "ip",
                                    self.public_ip_address)
                if self.public_ip_address:
                    frappe.get_doc(doctype, server).create_dns_record()
                frappe.db.set_value(doctype, server, "status",
    ...
    def disable_termination_protection(self):
        self.client().modify_instance_attribute(
            InstanceId=self.aws_instance_id, DisableApiTermination={
                "Value": False}
        )
        self.sync()
    ...
    def client(self, client_type="ec2"):
        ...
        return boto3.client(
            ...
            aws_secret_access_key=cluster.get_password(
    ...
    def create_server(self):
        ...
        return frappe.get_doc(document).insert(ignore_permissions=True)
    ...
    def get_security_groups(self):
        ...
        if self.series == "n":
            groups.append(
                frappe.db.get_value("Cluster", self.cluster,
...
def sync_virtual_machines():
    ...
    for machine in machines:
        try:
            ...
            frappe.db.rollback()
            log_error(title="Virtual Machine Sync Error",
...
def snapshot_virtual_machines():
    ...
    for machine in machines:
        try:
            ...
            frappe.db.rollback()
            log_error(title="Virtual Machine Snapshot Error",
                      virtual_machine=machine.name)
