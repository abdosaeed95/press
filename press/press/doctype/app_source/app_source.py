...
class AppSource(Document):
    ...
    def before_save(self):
        ...
        _, self.repository_owner, self.repository = self.repository_url.rsplit(
            "/", 2)
        # self.create_release()
    ...
    def create_release(self, force=False):
        ...
        try:
            ...
            if not frappe.db.exists(
                    "App Release", {"app": self.app,
                                    "source": self.name, "hash": hash}
            ):
                # frappe.db.count("App Release", {"app": self.name}) == 0
                is_first_release = 0
                frappe.get_doc(
                    {
                        "doctype": "App Release",
            ...
            github_response = github_response.text if (
                github_response is not None) else None
            frappe.db.set_value(
                "App Source",
                self.name,
                {
