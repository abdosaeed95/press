...
class AgentJob(Document):
    ...
    def retry_skip_failing_patches(self):
        ...
        updated_request_data = json.loads(
            self.request_data) if self.request_data else {}
        updated_request_data["skip_failing_patches"] = True
        self.request_data = json.dumps(
            updated_request_data, indent=4, sort_keys=True)

        return self.retry()

    def on_trash(self):
        steps = frappe.get_all("Agent Job Step", filters={
                               "agent_job": self.name})
        for step in steps:
...
def job_detail(job):
    ...
    for index, job_step in enumerate(
        ...
        step = {"name": job_step.step_name,
                "status": job_step.status, "index": index}
    ...
    if job.status == "Pending":
        ...
        current = {"name": job.job_type,
                   "status": job.status, "index": len(steps)}
...
def publish_update(job):
    ...
    frappe.publish_realtime(event="agent_job_update",
                            message=message, user=job_owner)
...
def suspend_sites():
    ...
    active_sites = frappe.get_all(
        "Site",
        filters={"status": "Active", "free": False,
                 "team": ("not in", free_teams)},
        fields=["name", "team", "current_database_usage",
                "current_disk_usage"],
    )
    ...
    for site in active_sites:
        if site.current_database_usage > 100 or site.current_disk_usage > 100:
            frappe.get_doc("Site", site.name).suspend(
                reason="Site Usage Exceeds Plan limits")
...
def poll_pending_jobs_server(server):
    ...
    random_pending_ids = random.sample(
        pending_ids, k=min(100, len(pending_ids)))
