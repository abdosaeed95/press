...
def get_ongoing_migration(site: str, scheduled=False):
    ...
    return frappe.db.exists(
        "Central Site Migration", {"site": site,
...
class CentralSiteMigration(Document):
    def before_insert(self):
        if get_ongoing_migration(self.site, scheduled=True):
            frappe.throw(
                "Ongoing/Scheduled Site Migration for that site exists.")

    @frappe.whitelist()
    def start(self):
        self.status = "Pending"
        self.save(ignore_permissions=True)
        frappe.enqueue_doc(self.doctype, self.name, "_start",
                           queue="long", timeout=2500)
    ...
    def _start(self):
        try:
            ...
            log_error("Central Site Migration Exception",
                      migration=self.as_dict())
            self.fail()
        self.save(ignore_permissions=True)
