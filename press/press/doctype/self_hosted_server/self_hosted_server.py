...
class SelfHostedServer(Document):
    ...
    def fetch_apps_and_sites(self):
        frappe.enqueue_doc(self.doctype, self.name,
                           "_get_apps", queue="long", timeout=1200)
        frappe.enqueue_doc(self.doctype, self.name,
                           "_get_sites", queue="long", timeout=1200)
    ...
    def append_to_apps(self):
        ...
        # Rearrange JSON if frappe isn't first app
        for i, app in enumerate(temp_task_result):
        ...
        try:
            for app in task_output:
                self.append(
                    "apps",
                    {"app_name": app["app"], "version": app["version"],
                     "branch": app["branch"]},
                )
                if app["app"] == "frappe":
                    self.frappe_version = self.map_branch_to_version(
                        app["branch"])
            self.status = "Active"
                ...
                if not frappe.db.exists("App Source", {"app": app["app"], "branch": app["branch"]}):
                    if not frappe.db.exists("App", {"_newname": app["app"]}):
                        ...
                        app_source_doc.append(
                            "versions", {"version": self.frappe_version})
                        app_source_doc.insert(ignore_permissions=True)
                        frappe.db.commit()
                        release_group.append(
                            "apps", {"app": app["app"], "source": app_source_doc.name})
                release_group.append(
                    "apps",
                    {
                        "app": app["app"],
    ...
    def create_db_server(self):
        try:
            ...
            db_server.mariadb_root_password = self.get_password(
            ...
            _db = db_server.insert(ignore_permissions=True)
    ...
    def create_server(self):
        ...
        try:
            ...
            server.self_hosted_mariadb_root_password = self.get_password(
                "mariadb_root_password")
            new_server = server.insert(ignore_permissions=True)
    ...
    def create_new_sites(self):
        ...
        try:
            for _site in self.sites:
                ...
                new_site.domain = frappe.db.get_list(
                    "Root Domain", pluck="name")[0]
                try:
                    new_site.bench = frappe.get_last_doc(
                        "Bench", {"group": self.release_group,
                                  "server": self.name}
                    ).name
                except Exception as e:
