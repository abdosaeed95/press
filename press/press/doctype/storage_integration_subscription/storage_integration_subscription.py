...
class StorageIntegrationSubscription(Document):
    ...
    def set_minio_server_on(self):
        ...
        self.minio_server_on = frappe.db.get_value(
            "Server", server, "proxy_server")

    def create_user(self):
        agent = Agent(server_type=self.SERVER_TYPE,
                      server=self.minio_server_on)
        data = {
...
def monitor_storage():
    ...
    for sub in active_subs:
        usage, unit_u = get_size(
            "bucket_name", sub["site"], access_key, secret_key)
        # not used yet
        if usage == 0:
            break
...
def get_analytics(**data):
    ...
    site, available = frappe.db.get_value(
        "Storage Integration Subscription", data["access_key"], [
            "site", "limit"]
    )
    access_key = frappe.db.get_value("Add On Settings", None, "aws_access_key")
    secret_key = get_decrypted_password(
