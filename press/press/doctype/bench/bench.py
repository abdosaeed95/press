...
class Bench(Document):
    ...
    def all_with_sites(fields=None, filters=None):
        benches = frappe.get_all(
            "Bench", filters=filters, fields=fields, pluck="name")
    ...
    def validate(self):
        if not self.candidate:
            candidate = frappe.get_all("Deploy Candidate", filters={
                                       "group": self.group})[0]
    ...
    def sync_analytics(self):
        ...
        for site, analytics in data.items():
            try:
                ...
                log_error("Site Analytics Sync Error",
                          site=site, analytics=analytics)
                frappe.db.rollback()
    ...
    def move_sites(self, server: str):
        try:
            ...
            frappe.throw(
                "Bench of corresponding Deploy Candidate not found in server")
            return
        sites = frappe.get_all(
            "Site", {"bench": self.name, "status": "Active"}, pluck="name")
...
class StagingSite(Site):
    ...
    def archive_expired(cls):
        expiry = frappe.db.get_single_value(
            "Press Settings", "staging_expiry") or 24
        sites = frappe.get_all(
            "Site",
            {"staging": True, "creation": (
                "<", datetime.now() - timedelta(hours=expiry))},
        )
    ...
    def create_if_needed(cls, bench: Bench):
        if not bench.staging:
            return
        try:
            cls(bench).insert(ignore_permissions=True)
...
def process_archive_bench_job_update(job):
    ...
    if updated_status != bench_status:
        frappe.db.set_value("Bench", job.bench, "status", updated_status)
        is_ssh_proxy_setup = frappe.db.get_value(
            "Bench", job.bench, "is_ssh_proxy_setup")
...
def archive_obsolete_benches():
    ...
    for bench in benches:
        ...
        for difference in differences:
            if frappe.db.exists(
                    "Bench", {"candidate": difference.destination,
                              "status": "Active"}
            ):
                try:
