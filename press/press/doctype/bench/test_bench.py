...
class TestStagingSite(unittest.TestCase):
    ...
    def test_create_staging_site(self):
        ...
        frappe.db.set_value(
            "Press Settings", None, "staging_plan", create_test_plan(
                "Site").name
        )
        count_before = frappe.db.count("Site")

        site = StagingSite(bench).insert(ignore_permissions=True)
...
class TestBench(unittest.TestCase):
    ...
    def _create_bench_with_n_sites_with_cpu_time(
        ...
        if not bench:
            ...
            create_test_subscription(
                site.name, plan.name, site.team)  # map site with plan
            bench = site.bench
            n -= 1
            count = 1
        else:
        ...
        for i in range(n):
            site = create_test_site(
                f"site-{count + i}-{self.bench_count}", bench=bench)
    ...
    def test_work_load_is_calculated_correctly(self):
        ...
        bench = self._create_bench_with_n_sites_with_cpu_time(
            3, 10, bench.name)
    ...
    def test_workers_get_allocated(self):
        bench = self._create_bench_with_n_sites_with_cpu_time(3, 5)
        workers_before = (bench.background_workers,
                          bench.gunicorn_workers)  # 1, 2
        scale_workers()
        bench.reload()
