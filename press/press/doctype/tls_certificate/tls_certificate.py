...
class TLSCertificate(Document):
    ...
    def _update_secondary_wildcard_domains(self):
        ...
        proxies_containing_domain = set(
            proxies_containing_domain) - set(proxies_using_domain)
    ...
    def trigger_site_domain_callback(self):
        domain = frappe.db.get_value(
            "Site Domain", {"tls_certificate": self.name}, "name")
        if domain:
            frappe.get_doc(
                "Site Domain", domain).process_tls_certificate_update()
    ...
    def _extract_certificate_details(self):
        ...
        self.issued_on = datetime.strptime(
            x509.get_notBefore().decode(), "%Y%m%d%H%M%SZ")
        self.expires_on = datetime.strptime(
...
def update_server_tls_certifcate(server, certificate):
    try:
        ...
        if server.doctype == "Proxy Server":
            proxysql_admin_password = server.get_password(
...
class LetsEncrypt(BaseCA):
    ...
    def run(self, command, environment=None):
        try:
            ...
            log_error("Certbot Exception", command=command,
