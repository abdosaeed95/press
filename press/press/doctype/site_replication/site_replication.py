...
class SiteReplication(Document):
    ...
    def validate_duplicate(self):
        ...
        if site_reps:
            frappe.throw(
                f"Site Replication for {self.site} is already running.")
    ...
    def get_all_running_site_replications(cls) -> List[Document]:
        replications = frappe.get_all(
            cls.doctype, dict(status="Running"), pluck="name")
...
def update_from_site():
    ...
    for replication in ongoing_replications:
        ...
        replication.save(ignore_permissions=True)
