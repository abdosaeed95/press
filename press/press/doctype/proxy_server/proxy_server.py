...
class ProxyServer(BaseServer):
    ...
    def validate_domains(self):
        ...
        for domain in domains:
            if not frappe.db.exists(
                ...
                frappe.throw(
                    f"Valid wildcard TLS Certificate not found for {domain}")
    ...
    def _setup_primary(self, secondary):
        secondary_private_ip = frappe.db.get_value(
            "Proxy Server", secondary, "private_ip")
        try:
            ansible = Ansible(
            ...
            self.status = "Broken"
            log_error("Primary Proxy Server Setup Exception",
                      server=self.as_dict())
        self.save(ignore_permissions=True)
    ...
    def _setup_secondary(self):
        ...
        try:
            ...
            self.status = "Broken"
            log_error("Secondary Proxy Server Setup Exception",
                      server=self.as_dict())
        self.save(ignore_permissions=True)
    ...
    def update_dns_record(self):
        try:
            ...
            self.status = "Broken"
            log_error("Route 53 Record Update Error",
    ...
    def _setup_proxysql_monitor(self):
        try:
            ...
            log_error("ProxySQL Monitor Setup Exception",
                      server=self.as_dict())
