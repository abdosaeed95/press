...
class DeployCandidateDifference(Document):
    def validate(self):
        ...
        source_creation = frappe.db.get_value(
            "Deploy Candidate", self.source, "creation")
    ...
    def populate_apps_table(self):
        ...
        destination_candidate = frappe.get_doc(
            "Deploy Candidate", self.destination)
        for destination in destination_candidate.apps:
            source = find(source_candidate.apps,
                          lambda x: x.app == destination.app)
            ...
            if not differences:
                ...
                difference = frappe.get_doc(
                    "App Release Difference", differences[0].name)
                difference.reload()
                difference.set_deploy_type()
            self.append(
                "apps",
