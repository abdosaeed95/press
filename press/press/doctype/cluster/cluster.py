...
class Cluster(Document):
    ...
    def provision_on_aws_ec2(self):
        ...
        client.modify_vpc_attribute(
            VpcId=self.aws_vpc_id, EnableDnsHostnames={"Value": True})
        ...
        try:
            ...
            response = client.import_key_pair(
                ...
                PublicKeyMaterial=frappe.db.get_value(
                    "SSH Key", self.ssh_key, "public_key"),
                TagSpecifications=[
    ...
    def get_all_for_new_bench(cls, extra_filters={}) -> List[Dict[str, str]]:
        cluster_names = unique(
            frappe.db.get_all("Server", filters={
                              "status": "Active"}, pluck="cluster")
        )
