...
class Agent:
    ...
    def new_site(self, site):
        ...
        database_server = frappe.db.get_value(
            "Bench", site.bench, "database_server")
        data = {
    ...
    def restore_site(self, site, skip_failing_patches=False):
        ...
        if site.remote_public_file:
            public_link = frappe.get_doc(
                "Remote File", site.remote_public_file).download_link
        if site.remote_private_file:
            private_link = frappe.get_doc(
                "Remote File", site.remote_private_file).download_link
        data = {
            "apps": apps,
            "mariadb_root_password": get_decrypted_password(
                "Database Server", database_server, "mariadb_root_password"
    ...
    def new_site_from_backup(self, site, skip_failing_patches=False):
        ...
        def sanitized_site_config(site):
            ...
            if site.remote_config_file:
                ...
                site_config = frappe.get_doc(
                    "Remote File", site.remote_config_file)
                new_config = site_config.get_content()
                # Don't allow deactivated sites to be created
                new_config["maintenance_mode"] = 0
    ...
    def backup_site(self, site, with_files=False, offsite=False):
        ...
        if offsite:
            ...
            bucket_name = (
                backup_bucket.get("name") if isinstance(
                    backup_bucket, dict) else backup_bucket
            )
            if settings.aws_s3_bucket or bucket_name:
                ...
                data.update(
                    {"offsite": {"bucket": bucket_name,
                                 "auth": auth, "path": backups_path}}
                )
    ...
    def update_upstream_private_ip(self, server):
        ip, private_ip = frappe.db.get_value(
            "Server", server, ["ip", "private_ip"])
    ...
    def request(self, method, path, data=None, files=None):
        try:
            ...
            if frappe.conf.developer_mode and intermediate_ca:
                ...
                verify = frappe.get_doc(
                    "Certificate Authority", root_ca).certificate_file
            else:
                verify = True
            if files:
                file_objects = {
                    key: frappe.get_doc(
                        "File", {"file_url": url}).get_content()
                ...
                result = requests.request(
                    method, url, headers=headers, json=data, verify=verify, timeout=(
                        10, 30)
                )
            json_response = None
            try:
    ...
    def update(self):
        url = frappe.get_doc(
            self.server_type, self.server).get_agent_repository_url()
    ...
    def fetch_site_status(self, site):
        data = self.get(
            f"benches/{site.bench}/sites/{site.name}/status")["data"]
        return data
