...
class Ansible:
    def __init__(self, server, playbook, user="root", variables=None, port=22):
        ...
        self.playbook_path = frappe.get_app_path(
            "press", "playbooks", self.playbook)
        ...
        context.CLIARGS = ImmutableDict(
            ...
            extra_vars=[f"{key}='{value}'" for key,
                        value in self.variables.items()],
            remote_user=user,
            start_at_task=None,
            syntax=False,
            verbosity=3,
    ...
    def patch(self):
        ...
        def modified_poll_async_result(executor, result, templar, task_vars=None):
            job_id = result["ansible_job_id"]
            task = executor._task
            self.callback.on_async_start(
                task._role.get_name(), task.name, job_id)
        ...
        for role in play.get_roles():
            for block in role.get_task_blocks():
                for task in block.block:
                    ...
                    self.tasks.setdefault(role.get_name(), {})[
                        task.name] = task_doc.name
